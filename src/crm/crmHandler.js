...
const INTERVALO_WHATSAPP = 4000; // 4s entre mensagens
...
async function disparar(lista, gatilho) {
  ...
  for (const lead of lista) {
    ...
    try {
      ...
      await gravarLog({ contexto: 'crm', mensagem: `Erro ${gatilho}`, telefone: lead.telefone, payload: { erro: e.message } });
    }
  }
...
export async function rodarCRM() {
  ...
  for (const g of gatilhos) {
    try {
      ...
      await sleep(1000); // pausa entre gatilhos
    } catch (e) {
